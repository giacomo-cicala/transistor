...
Dalle caratteristiche di uscita del transistor per le due correnti di base riportate in (\cref{fig:fit_caratteristiche}), si sono eseguiti due fit lineari nella regione attiva del transistor (\err{scegli})
\begin{equation}
    I_{C}= a+bV_{CE}
\end{equation}
...
\begin{figure}[hbtp]
    ...
    \caption{Caratteristiche di uscita del transistor per le correnti di base $I_{B}=50 \mu A$ (in blu) e $I_{B}=100 \mu A$ (in rosso) con i relativi fit lineari nella regione attiva. Sulle ascisse sono riportate le tensioni collettore-emettitore, sulle ordinate le correnti di collettore.}
...
\begin{equation*}
    a_{50} = (\cdots \pm \cdots) \text{ mV} \hspace{2cm} b_{50} = (\cdots \pm \cdots) \text{k}\Omega^{-1}
\end{equation*}
\begin{equation*}
    a_{100} = (\cdots \pm \cdots) \text{ mV} \hspace{2cm} b_{100} = (\cdots \pm \cdots) \text{k}\Omega^{-1}
\end{equation*}

\noindent
da cui si ottengono i valori di tensione di Early

\begin{equation*}
    V_{A,50} = (\cdots \pm \cdots) \text{ V} \hspace{2cm} V_{A,100} = (\cdots \pm \cdots) \text{ V}
\end{equation*}
...
\noindent
dove $\Delta I_{C}$ è la differenza tra le correnti di collettore misurate per le due correnti di base alla stessa tensione di collettore, e $\Delta I_{B}$ è la differenza tra le due correnti di base. Scegliendo come tensione di collettore (\err{decidi}) V, si ottiene

\begin{equation*}
    \beta = \cdots
\end{equation*}

\err{DISCUSSIONE RISULTATI}
