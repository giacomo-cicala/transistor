...
\begin{abstract}\noindent
	Sono state svolte delle misure su un transistor PNP in configurazione emettitore comune.
...
parametro $b$ invece rappresenta la resistenza del ramo del collettore con
$I_b$ fissato (\err{nome specifico?}). La conduttanza, definita come:
\begin{equation}
...
\begin{figure}
	...
	\begin{circuitikz}
		...
		\draw (base) to [rmeterwa, t=A, i=$i_b$] (pot_b.wiper -| base) -- (pot_b.wiper);
	\end{circuitikz}
	\qquad
	\begin{circuitikz}
		% POWER RAILS
		...
		\draw (T.base) -- ++(1,0) coordinate (base);
		% Collector
		\draw (T.collector) to [short, -*] ++(0,0.5) coordinate (collector) to [rmeterwa, t=A, i=$i_c$] (T.collector |- pot_c.wiper) -- (pot_c.wiper);
		\draw (collector -| 0,0) to[short,*-] ++(1.5,0) coordinate (tmp) to [rmeterwa, t=V, v=$v_c$] (collector);
...
\'E stato realzzato il circuito di \cref{fig:circuito}.
I due potenziometri $R_B$ e $R_C$ funzionano da partitori di tensione,
comportandosi come due sensibili generatori di tensione variabile che ci permettono di impostare le correnti di base e collettore.
Il potenziometro collegato al collettore ha resistenza totale $R_B = 10 k$ e
I componenti scelti sono:
\begin{itemize}
	\item Transistor PNP: \texttt{\err{codice}}
	\item Potenziometro $R_B = 100 k$
\end{itemize}
Come prima cosa è stata impostata la corrente di base, v

\section*{Risultati e discussione}
Si riportano in (\cref{tab:50uA}) e in (\cref{tab:100uA}) le misure delle coppie di valori I-V rispettivamente per corrente di base di $-50$ \mu A e $-100$ \mu A. I valori di tensione e corrente sono già stati cambiati di segno per ottenere le caratteristiche nel primo quadrante Per il calcolo degli errori associati alle misure di tensione e corrente si rimanda all'\cref{appendice:errori_I-V}. Le caratteristiche di uscita del transistor per le due correnti di base
considerate sono riportate in (\cref{fig:caratteristiche}). 
...
Per ottenere delle stime per la tensione di Early e per la conduttanza,  si sono eseguiti dei fit lineari come in \cref{eq:VA} nella regione attiva del transistor, compresa tra $1$ V e $3.5$ V. I valori dei parametri di fit ottenuti per le due correnti di base sono

\begin{equation*}
	a_{50} = (22 \pm 3) \text{ V} \hspace{2cm} b_{50} = (2.3 \pm 0.3) \text{ k\Omega}
\end{equation*}
\begin{equation*}
	a_{100} = (16.0 \pm 1.3) \text{ V} \hspace{2cm} b_{100} = (0.95 \pm 0.07) \text{ k\Omega}
\end{equation*}
Dunque, per le considerazioni già fatte, abbiamo che le due stime per la tensione di Early e per la conduttanza sono

\begin{equation*}
	V_{a,50} = (22 \pm 3) \text{ V} \hspace{2cm} V_{a,100} = (16.0 \pm 1.3) \text{ V}
\end{equation*}
\begin{equation*}
    g_{50} = (0.43 \pm 0.05) \text{ mS} \hspace{2cm} g_{100} = (1.05 \pm 0.07) \text{ mS}
\end{equation*}

\noindent
le cui incertezze sono state calcolate come descritto nell'\cref{appendice:errori_conduttanza}.

Tramite l' \cref{eq:beta}, scegliendo come tensione di collettore $-V_{CE} = 3$
V, si ottiene una stima del guadagno di corrente del transistor pari a
\begin{equation*}
	\beta = 190.6
\end{equation*}
...
sperimentale. Nonostante ciò, il valore della conduttanza per la corrente di base di $-100$ \mu A risulta maggiore rispetto a quella di $-50$
\mu A, \err{compatibilmente col fatto che la tensione di Early debba essere unica per tutte le correnti, e che dunque la pendenza della retta della caratteristica in regione attiva debba essere maggiore per correnti più alte.}
\section*{Conclusioni}

I valori della tensione di Early ottenuti per le due correnti di base a $-50$
\mu A e a $-100$ \mu A sono
\begin{equation*}
	V_{a,50} = (22 \pm 3) \text{ V} \hspace{2cm} V_{a,100} = (16.0 \pm 1.3) \text{ V}
\end{equation*}
risultando non compatibili tra loro entro l'incertezza sperimentale. Le stime per la conduttanza
Invece, la stima del guadagno di corrente del transistor ottenuta scegliendo
come tensione di collettore $-V_{CE} = 3$ V è pari a
\begin{equation*}
	\beta = 190.6
\end{equation*}
...
L'errore sulla conduttanza $g$ è stato calcolato tramite propagazione degli errori a partire dall'\cref{eq:conduttanza} come
\begin{equation}
